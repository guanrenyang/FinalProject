...
void MainWindow::drawProgressBar()
{
    ...
    QLabel *loadStatus=new QLabel(progressBar);
    loadStatus->setText("Loading ...");

    progressBar->setMinimum(0);
    progressBar->setVisible(true);

    connect(selectWindow,SIGNAL(emitAllFilesNumber(int)),progressBar,SLOT(setMaximum(int)));
    connect(loadDataThread,SIGNAL(emitFileNumber(int)),progressBar,SLOT(setValue(int)),Qt::QueuedConnection);
...
void MainWindow::InitializeGridData(StartEnd startEnd)
{
    ...
    gridStartPerHour.resize(100);
    gridEndPerHour.resize(100);
    gridFeesPerHour.resize(100);

    for(int i=0;i<gridStartPerHour.size();i++)
    {
        gridStartPerHour[i].resize(allHours);
    }
    for(int i=0;i<gridEndPerHour.size();i++)
    {
        gridEndPerHour[i].resize(allHours);
    }
    for(int i=0;i<gridFeesPerHour.size();i++)
    {
        gridFeesPerHour[i].resize(allHours);
    }
}
...
{
    qint64 oneRecordStartHour=oneRecordStartDateTime.secsTo(*endDateTime)/3600+1;
    qint64 oneRecordEndHour=oneRecordEndDateTime.secsTo(*endDateTime)/3600+1;

    if(
            0<=startGrid&&startGrid<=99 &&
            0<=endGrid&&endGrid<=99 &&
            oneRecordStartHour<allHours &&
            oneRecordEndHour<allHours
            )
    {

        gridStartPerHour[startGrid][0]+=1;
        gridFeesPerHour[startGrid][0]+=fees;

        gridEndPerHour[endGrid][0]+=1;
        gridFeesPerHour[endGrid][0]+=fees;
    }
}
void MainWindow::ShowGraph()
{
    ...
    widget->setWindowTitle(QStringLiteral("Didi Taxi Damand Analyzer (20161101-2016115)"));

    //"Change theme combox box"
